...
import ventas.service.BaseServiceImpl;
...
public abstract class BaseControllerImpl<E extends Base,S extends BaseServiceImpl<E,Long>> implements BaseController<E,Long> {
    ...
    //  ResponseEntity nos brinda la respuesta en formato Json
    @GetMapping("")
    ...
    public ResponseEntity<?> getId(@PathVariable Long id){
        try {
            ...
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("{\"error\":\"Error. ID no encontrado.\"}");
        }
    }
    @PostMapping("")
    ...
    public ResponseEntity<?> delete(@PathVariable Long id){
        try {
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("{\"error\":\"Error. No se pudo borrar\"}");
        }
    }
